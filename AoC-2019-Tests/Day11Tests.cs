...
namespace Tests
{
    ...
    public class Day11Tests
    {
        ...
        public void MovementTest()
        {
            ...
            var hullBody = new HullBody();
            paintingRobot.PaintAndTurn(new [] {1,0}, hullBody);
            Assert.That(paintingRobot.Direction == 180);
            Assert.That(hullBody.Body[startingX, startingY] == 1);
        }

        [Test]
        public void CorrectlyCalculatesPaintedPanels()
        {
            const string inputProgram =
            ...
            var robotFactory = new PaintingRobotFactory();
            var paintingService = new PaintingService();
            var hullBody = new HullBody();
            var robot = robotFactory.Create(inputProgram);
            paintingService.RunPaintingProgram(robot, hullBody);
            Assert.That(robot.PaintedPanels.Count == 1934);
        }
    }
}